...
export default class App extends Component {
  ...
  componentWillMount(){
    const userListStreams = this.state.usersList.map((user) => {
      return axios.get(`https://api.twitch.tv/kraken/users/${user}?client_id=${TWITCH_API}`)
      .then(function (response) {
        return axios.get(`https://api.twitch.tv/kraken/streams/${user}?client_id=${TWITCH_API}`)
      })
    ...
    const userListTwitch = this.state.usersList.map((user) => {
      return axios.get(`https://api.twitch.tv/kraken/users/${user}?client_id=${TWITCH_API}`).catch(function (error) {
        return {error: 'error'};
      });
    })
